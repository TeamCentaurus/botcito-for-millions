...
resource "google_project_iam_member" "cloud-function-sa-roles" {
...
resource "google_pubsub_topic" "stock_tick_topic" {
  name = "stock-request-topic"

  labels = local.common_labels
}

resource "google_cloud_scheduler_job" "stock_scheduler" {
  name             = "stock-update-scheduler"
  description      = "Activa la actualización de acciones cada minuto de L-V"
  schedule         = "* 9-16 * * 1-5"
  ...
  pubsub_target {
    ...
    data       = base64encode("start_process")
  }
...
resource "google_artifact_registry_repository" "stock_repo" {
  location     = var.region
  repository_id = "stock-functions-repo"
  description  = "Docker repository for stock Cloud Function"
  format       = "DOCKER"
